...
Mains loss (blackout) handling
Why B0 is not used for state
Cross-check against gregrenda
Keypad-specific mapping
Hardware
Powering the ESP32
Compatibility
Contributing
...
gregrenda/ademco reverse-engineered the same non-addressable bus for the 6150 keypad — which is also fixed-word, like the 4127 here, just a different model with a different segment layout. Converting his bit constants to byte.bit positions:
