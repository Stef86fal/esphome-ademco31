...
static const uint32_t   RMT_RESOLUTION_HZ = 1000000;  // 1 tick = 1us (come prima: 80MHz/80)
...
class Ademco4110Component : public Component, public uart::UARTDevice {
 public:
  ...
  void set_zone_changed_sensor(text_sensor::TextSensor *s) { zone_changed_sensor_ = s; }
 ...
 protected:
  ...
  bool        bypass_prev_{false};  // per rilevare la transizione ON->OFF del bypass
  uint16_t    bypass_zone_mask_{0};  // bit 0 = zona 1 ... bit 7 = zona 8, toggle ad ogni rilevazione
  binary_sensor::BinarySensor *bus_ok_sensor_{nullptr};
  bool        bus_silent_{false};  // stato pubblicato del sensore Comunicazione Bus
  text_sensor::TextSensor     *status_sensor_{nullptr};
  text_sensor::TextSensor     *zones_sensor_{nullptr};
  text_sensor::TextSensor     *alarm_zone_sensor_{nullptr};
  text_sensor::TextSensor     *zone_changed_sensor_{nullptr};
  ...
  void publish_bypass_zones();             // pubblica l'elenco zone in bypass_zone_mask_
