...
@automation.register_action(
    "ademco4110.send_keys", SendKeysAction, SEND_KEYS_ACTION_SCHEMA,
    # play() accoda i tasti e ritorna subito: l'invio vero avviene dopo, in
    # modo asincrono, dentro sync_task(). Stesso comportamento del default
    # implicito precedente, ora dichiarato esplicitamente.
    synchronous=False,
)
...
@automation.register_action(
    "ademco4110.scan_zones", ScanZonesAction, SCAN_ZONES_ACTION_SCHEMA,
    # Stesso motivo: scan_zones() avvia una scansione che dura ~14s in modo
    # asincrono, play() non ne attende il completamento.
    synchronous=False,
)
...
async def to_code(config):
    # ESPHome >= 2026.2 esclude di default i componenti IDF non usati:
    # il driver RMT nuovo (driver/rmt_tx.h) va riattivato esplicitamente,
    # altrimenti la build fallisce con "rmt_encoder.h: No such file or directory".
    try:
        from esphome.components.esp32 import include_builtin_idf_component
        include_builtin_idf_component("esp_driver_rmt")
    except ImportError:
        pass  # ESPHome < 2026.2: tutti i componenti IDF inclusi di default
