...
- [Features](#-features)
- [A note on language](#-a-note-on-language-italian-strings)
...
- [Configure this for your alarm (code + zone names)](#️-configure-this-for-your-alarm-code--zone-names)
...
This firmware was written for the author's own installation, so some strings are in **Italian** — most visibly the entity names / `entity_id`s it generates, the `ZONE_NAMES` array, and the `ESP_LOGx` log messages. **None of this affects how the firmware works** — the bus decoding is identical whatever the labels say; the Italian text is purely cosmetic labelling.

If you're not Italian, you'll want to change:

- **`entity_id` / entity names** — rename them in the ESPHome YAML (or in Home Assistant after import) to match your own naming scheme; you'd typically do this anyway.
- **`ZONE_NAMES` in `ademco4110.h`** — your own zone labels regardless of language (see the configuration section).
- **Log messages** (optional) — the `ESP_LOGx` strings in `ademco4110.cpp` are in Italian; translate them only if you want English logs.

Treat the Italian strings as defaults to be localised, the same way you'd adapt zone names to your own house. PRs that add an English baseline or make the strings configurable are welcome.
...
## ⚙️ Configure this for your alarm (code + zone names)

Before flashing you must adapt two things to your own system: your **secret alarm code** and your **zone names**. Both are clearly marked in the config.

### 1. Your alarm code (required)

The published YAML has the code blanked out as asterisks (`****`) in **two places** — you must replace **both** with your own user code, or arming/disarming won't work.

**a) Arm / disarm commands** (top of `ademco4110.yaml`, in `substitutions:`). Each is your code followed by the panel's function key (1 = off, 2 = total, 3 = partial on this panel):

```yaml
substitutions:
  # Replace **** with your 4-digit user code
  alarm_arm_total:   "****2"   # code + 2  (arm away/total)
  alarm_arm_partial: "****3"   # code + 3  (arm stay/partial)
  alarm_disarm:      "****1"   # code + 1  (disarm/off)
```

So if your code is `1234`, these become `"12342"`, `"12343"`, `"12341"`.

**b) Open-zone scan** (in the `ademco4110:` component block). The scan presses `*` to make the panel scroll open zones, then disarms out of that view — so it needs your code too:

```yaml
ademco4110:
  # Replace **** with your 4-digit user code (WITHOUT the function key here)
  # e.g. if your OFF sequence is "12341", put "1234"
  disarm_code: "****"
```

> ⚠️ Your alarm code is a **secret**. If you publish or share your config, make sure these stay blanked (`****`) — never commit the YAML with your real code filled in.

### 2. Your zone names

The panel scrolls zone **numbers** (1–8); the names are mapped in one array. Edit **`ZONE_NAMES` in `ademco4110.h`**:
...
Replace the strings with your own labels, keeping position = zone number (index 0 stays empty; zones are numbered from 1). The values shown are the author's layout — yours will differ. Recompile and reflash after editing.

> Note: zones 3 and 5 here are the interior PIRs the panel auto-excludes on **Partial/Stay** arming — that's why their numbers scroll on the bus during a partial arm (see the contextual-zone-read note). Check which of your zones are the interior motion detectors.

The function keys (1/2/3) and zone count may differ on your panel/programming — verify against your own keypad before relying on the commands.
...
This repo does **not** include a `secrets.yaml` — you create it yourself before compiling, and it stays out of the repo (add it to `.gitignore`). The main config references these values with `!secret`, so ESPHome expects a `secrets.yaml` next to it containing:

```yaml
# secrets.yaml — create this yourself; do NOT commit it
wifi_ssid:          "your-wifi-name"
wifi_password:      "your-wifi-password"
wifi_ap_password:   "your-ap-fallback-password"   # fallback hotspot
api_encryption_key: "your-esphome-api-key"         # generated by ESPHome
ota_password:       "your-ota-password"
```

Generate the API key and OTA password with ESPHome, or reuse your existing ones. (The alarm code is **not** here — it lives in the YAML `substitutions:`, see the configuration section above.)

> Because `secrets.yaml` holds your WiFi password, it must never be committed. Keeping it out of the repo entirely (this approach) is the safest option — there's no file that could accidentally be pushed with real values. Note your **alarm code** lives in the YAML `substitutions:` (blanked as `****`), so take the same care there: don't commit the YAML with your real code filled in. If you'd rather have a template in the repo, commit a `secrets.yaml.example` with placeholder values and `.gitignore` the real `secrets.yaml`.

Also remember to set your **static IP** (or DHCP reservation) in the `wifi:` block — the published config has network details blanked so the author's topology isn't exposed.
