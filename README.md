...
The published YAML has the static IP settings removed/blanked — you must set your own before flashing. Look for the wifi: section and fill in the manual_ip: block with values valid for your network:
yamlwifi:
  ssid: !secret wifi_ssid
  password: !secret wifi_password
  manual_ip:
    static_ip: 192.168.1.XXX    # ← choose a free IP on your LAN
    gateway: 192.168.1.1        # ← your router
    subnet: 255.255.255.0
    # dns1: 192.168.1.1         # optional
A static IP is recommended for a security-related device so Home Assistant always reaches it at a known address (and so a DHCP lease change never drops the connection). Make sure the address you pick is outside your router's DHCP pool, or reserve it in the router, to avoid conflicts.
If you'd rather use DHCP, just omit the manual_ip: block entirely — but then set a DHCP reservation on your router so the device keeps a stable address.

Credentials (wifi_ssid, wifi_password) should live in ESPHome's secrets.yaml, never hard-coded in the committed config — keep them out of your public repo.


Ho aggiunto anche la riga finale sui secrets perché è lo stesso tipo di problema di privacy: assicurati che nel repo pubblico non finiscano SSID e password WiFi in chiaro — vanno in secrets.yaml, che tieni fuori dal repo (nel .gitignore). Se non l'hai già fatto, controlla che il file committato usi !secret e non le credenziali vere, come per l'IP.
Se vuoi lo aggiungo anche alla tabella dei contenuti: - [Network configuration](#-network-configuration-static-ip). Fammi sapere se lo vuoi più stringato.scrivi proprio una parte di readme che dice che va impostato un secrets.yaml con dentro tutte le info che servono al firmwareEcco la sezione sul secrets.yaml da aggiungere al README:

🔐 Secrets (secrets.yaml)
Sensitive values must not be committed to a public repo. ESPHome keeps them in a separate secrets.yaml file that sits next to your config and is referenced with !secret. Create your own secrets.yaml (and add it to .gitignore) with all the values this firmware needs:
yaml# secrets.yaml — DO NOT commit this file
wifi_ssid: "YourWiFiName"
wifi_password: "YourWiFiPassword"

# API / OTA (ESPHome)
api_encryption_key: "your-32-byte-base64-key"   # generated by ESPHome
ota_password: "your-ota-password"

# Fallback hotspot (optional, if you use the AP fallback)
ap_password: "your-ap-password"
In the main YAML these are pulled in with !secret, for example:
yamlwifi:
  ssid: !secret wifi_ssid
  password: !secret wifi_password
...
Add secrets.yaml to your .gitignore so it never lands in the repo:
# .gitignore
secrets.yaml

The committed config in this repo references !secret placeholders only — it contains no real credentials. You supply your own secrets.yaml locally before compiling. Never paste real SSIDs, passwords, API keys, IP addresses, or other network details into files you push publicly.

Adjust the exact key names to match whatever !secret references your YAML actually uses — the list above covers the typical set (WiFi, API encryption, OTA, and the optional AP fallback password).
